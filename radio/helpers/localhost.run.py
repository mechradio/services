...
# Paths, but looking from radio.py
...
def notify_radio(url):
    data = {
        'url': url,
        'token': token
    }
    r = requests.post('http://127.0.0.1:8000/changeUrl', data=data)
    if r.ok: print("[HELPER] Radio received url change info", flush=True)
    else: print("[HELPER] Failed to notify radio about url change", flush=True)

# Loop
while True:
    result = fetch_until_pattern_found()
    # If result found
    if(result): notify_radio(result)
    else: time.sleep(1)
